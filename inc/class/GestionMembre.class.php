        ...
        if($element)
        {
            ...
            if(empty($contenu))
            {
                ...
                else
                {
					...
					{
						echo 'Mot de passe incorrects';
					}else {
						$pass = hash('sha3-512', $element['mdp']);
                        executeRequete("INSERT INTO membre (pseudo, mdp, nom, prenom, email, civilite, ville, code_postal, adresse, photo) VALUES ('$element[pseudo]', '$pass', '$element[nom]', '$element[prenom]', '$element[email]', '$element[civilite]', '$element[ville]', '$element[code_postal]', $element[photo], '$element[adresse]')");
